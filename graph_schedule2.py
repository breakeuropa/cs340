# ----------------------------
# Module 3: Delivery Scheduler + History + Undo (DROP-IN)
# ----------------------------
...
class DeliveryTask:
    ...
    def __init__(self, route: str, time_min: int):
        # Initialize the route and time attributes
        self.route = route
        # Convert time from minutes to "H:MM" format
        self.time = time_min

class _QNode:
    # Represents a node in the delivery queue
    def __init__(self, task: DeliveryTask):
        # Initialize the task and next attributes
        self.task = task
...
class HistoryLog:
    def __init__(self):
        # list of DeliveryTask sorted by time
        self.items: list[DeliveryTask] = []

    def insert_sorted(self, task: DeliveryTask) -> None:
        # simple insertion to keep sorted by time (stable)
        i = 0
        while i < len(self.items) and self.items[i].time <= task.time:
            i += 1
        self.items.insert(i, task)

    def remove_exact_tasks(self, tasks: list[DeliveryTask]) -> None:
        # remove each (route,time) once
        for t in tasks:
            ...
            for raw in file:
                ...
                # UNDO_LAST
                    ...
                    if kind == "SCHEDULE":
                        ...
                        # remove those tasks from history
                        history.remove_exact_tasks(moved)
                        # restore them back to queue in original order
                        for t in moved:
