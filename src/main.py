...
class TreeNode:
    ...
    def __init__(self, src_to_dst: str, time: int) -> None:
    ...
    def __str__(self) -> str:
        t: str = f"{self.time:04}"[:2] + ":" + f"{self.time:04}"[2:]
        return f"{self.src_to_dst} at {t}"
...
class BinaryTree:
    ...
    def insert(self, root: TreeNode, node: TreeNode) -> TreeNode:
        ...
        else: root.right = self.insert(root.right, node)
    ...
    def remove_previous(self) -> None:
        ...
        size: int = len(self.history_stack)
        root: TreeNode = self.history_stack[-1]
        
        if size >= 2:
            before_root: TreeNode = self.history_stack[-2]
            
            if before_root.left == root:
                before_root.left = None
            elif before_root.right == root:
                before_root.right = None
                
        elif size == 1: self.root = None
                
        self.history_stack.pop()
            
    
        
    def print_history(self, root: TreeNode, start_time: int, end_time: int) -> None:
        
        if root == None:
            return None
        
        self.print_history(root.left, start_time, end_time)
        
        if root.time >= start_time and root.time <= end_time:
            print(f"- {root}")
            
        self.print_history(root.right, start_time, end_time)
            
            
# ----------------------------
# Queue 
# ----------------------------
class QueueNode: # Holds city name and time
        ...
        with open(srcfile, "r") as file:
            ...
            for line in file:
                ...
                if values[0] == "SCHEDULE" and values[1] == "DELIVERY":
                    ...
                    if len(values[4]) != 5: # 00:00
                        continue
                                        
                    try: time: int = int(values[4].replace(":", ""))
                    except: continue
                    
                    delivery.enqueue(QueueNode(city, time))
                    
                elif values[0] == "RECORD_HISTORY":
                    
                    while (not delivery.is_empty()):
                        data: QueueNode = delivery.dequeue()
                        history.insert(history.root, TreeNode(data.name, data.time))
                    
                elif values[0] == "QUERY_HISTORY":
                    
                    if len(values) != 4:
                        continue
                    
                    if len(values[2]) != 5 or len(values[3]) != 5:
                        continue
                    
                    print(f"History between {values[2]} and {values[3]}:")
                    
                    try: 
                        start: int = int(values[2].replace(":", ""))
                        end: int = int(values[3].replace(":", ""))
                    except: continue
                    
                    history.print_history(history.root, start, end)
                    
                elif values[0] == "UNDO_LAST":
                    history.remove_previous()
